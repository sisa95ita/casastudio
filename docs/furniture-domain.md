...
Canonical parsing does not require anchor or rotated-footprint containment. A valid Room relationship is mandatory, but neither the anchor nor full footprint must pass a polygon containment test at the domain boundary. Draft Rooms with empty boundaries remain supported. The [2D authoring workflow](furniture-2d-authoring.md) separately requires anchor containment when placing, moving, or reassigning Furniture. There is no collision, snapping, clearance or Project-wide footprint overlap constraint.
...
No Furniture runtime geometry is introduced. The existing immutable GeometryModel derives topology and architectural spatial facts used by current consumers. The 2D viewer derives a separate renderer-neutral Furniture presentation directly from canonical instances; vertical placement remains available through the pure Room resolver. Geometry Snapshot and Geometry API contracts therefore remain unchanged. Canonical Furniture carries no SVG, mesh, transform object, asset URL, or renderer state.
