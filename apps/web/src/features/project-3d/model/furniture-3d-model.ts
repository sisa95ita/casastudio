...
} from "../assets/furniture-asset-registry";
import {
...
export function canAdaptFurnitureAsset(
  item: FurnitureItem,
  asset: FurnitureAssetDefinition
): boolean {
  ...
  const ratios = [
    item.width / definition.defaultWidth,
    item.depth / definition.defaultDepth,
    item.height / definition.defaultHeight
  ];
  return (
    Math.max(...ratios) / Math.min(...ratios) <= asset.deformation.maxAnisotropy
  );
}

/** Normalizes native bounds after orientation correction, then fits each physical axis exactly. */
export function furnitureAssetTransform(
  asset: FurnitureAssetDefinition,
  target: Pick<FurnitureModel3D, "width" | "depth" | "height">
) {
  ...
  return Object.freeze({
    ...
    scale: Object.freeze([
      target.width / bounds.size.x,
      target.height / bounds.size.y,
      target.depth / bounds.size.z
    ] as const),
    yaw: asset.yaw
  });
}
...
export function createFurnitureModels3D(
  project: Project
): readonly FurnitureModel3D[] {
  return Object.freeze(
    project.building.furniture.flatMap((item) => {
      const model = createFurnitureModel3D(project, item);
      return model ? [model] : [];
    })
  );
}

/** Derives one ephemeral or canonical furnishing without changing the owning Project. */
export function createFurnitureModel3D(
  project: Project,
  item: FurnitureItem
): FurnitureModel3D | undefined {
  const owner = resolveFurnitureRoom(project, item);
  if (!owner) return undefined;
  const definition = resolveBuiltinFurnitureDefinition(item.definitionId);
  const position = projectPointToThree(
    item.position,
    owner.floorElevation,
  ...
  return Object.freeze({
    ...
    yaw,
    asset:
      resource && canAdaptFurnitureAsset(item, resource) ? resource : undefined,
