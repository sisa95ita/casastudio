...
import {
  furnitureAssetTransform,
  type FurnitureModel3D
} from "./model/furniture-3d-model";
...
export class FurnitureAssetBoundary extends Component<
...
export function FurnitureAsset3D({
  model,
  state,
  invalid = false
}: {
  model: FurnitureModel3D;
  ...
  return (
    ...
    >
      {model.asset ? (
        <FurnitureAssetBoundary key={model.asset.resource} fallback={fallback}>
          <Suspense fallback={fallback}>
            <LoadedFurnitureAsset model={model} asset={model.asset} />
          </Suspense>
        </FurnitureAssetBoundary>
      ) : (
        fallback
      )}
      {state !== "idle" && (
        <mesh position={[0, model.height / 2, 0]} raycast={() => undefined}>
...
function LoadedFurnitureAsset({
  ...
  const transform = useMemo(
    () => furnitureAssetTransform(asset, model),
    [asset, model]
  );
  ...
  return (
    <group scale={[...transform.scale]} dispose={null}>
      <group position={[...transform.offset]}>
        <group rotation={[0, transform.yaw, 0]}>
...
function FurnitureFallback3D({ model }: { model: FurnitureModel3D }) {
  ...
  return (
    <group name="furniture-fallback">
      <RoundedBox
        args={[model.width, model.height - plinth, model.depth]}
        radius={radius}
        smoothness={2}
        position={[0, (model.height + plinth) / 2, 0]}
        material={furnitureFallbackMaterial3D}
        castShadow
        receiveShadow
        dispose={null}
      />
      <mesh
        ...
        material={furnitureFallbackPlinthMaterial3D}
        castShadow
        receiveShadow
        dispose={null}
      >
