...
import type { SvgViewportPoint, WorldPointXZ } from "../../../geometry-2d/viewport/viewport-transform-2d";
...
export const drawWallSnapPriority = Object.freeze([
  "vertex", "wall-endpoint", "wall-midpoint", "wall-intersection",
  "wall-interior", "orthogonal", "grid", "free"
] as const);
...
export type DrawWallSnapCandidate =
  ...
  | (SnapBase & { readonly kind: "wall-intersection"; readonly wallIds: [string, string] })
  | (SnapBase & { readonly kind: "wall-interior"; readonly wallId: string })
  | (SnapBase & { readonly kind: "orthogonal"; readonly axis: "horizontal" | "vertical" })
...
export function resolveDrawWallSnapCandidate(
  ...
  const options: DrawWallSnapOptions = typeof rawOptions === "number"
    ? { cssPixelsPerSvgUnit: rawOptions, tolerancePixels: legacyTolerancePixels }
    : rawOptions;
  ...
  wallEdges.forEach((first, firstIndex) => {
    wallEdges.slice(firstIndex + 1).forEach((second) => {
      ...
      intersections.push({
        ...
        wallIds: [first.sourceWallId, second.sourceWallId].sort() as [string, string],
        point: interpolate(first.start.world, first.end.world, found.firstParameter),
        visualDistancePixels: distance * cssScale
      });
    });
  });
  ...
  const wallInterior = chooseNearest(wallEdges.flatMap((edge) => {
    ...
    return [{
      ...
      wallId: edge.sourceWallId,
      point: interpolate(edge.start.world, edge.end.world, projection.parameter),
    ...
    const orthogonal = [
      {
        ...
        visualDistancePixels: Math.abs(pointer.y - options.drawStart.svgPoint.y) * cssScale
      },
      {
        kind: "orthogonal" as const,
        ...
        visualDistancePixels: Math.abs(pointer.x - options.drawStart.svgPoint.x) * cssScale
      }
...
function chooseNearest<T extends DrawWallSnapCandidate>(candidates: readonly T[]): T | undefined {
  return [...candidates].sort((first, second) => {
    ...
    return Math.abs(difference) > drawWallSnapConfiguration.distanceTieTolerancePixels
      ? difference
      : first.geometryId.localeCompare(second.geometryId);
  })[0];
}

function projectOntoSegment(pointer: SvgViewportPoint, start: SvgViewportPoint, end: SvgViewportPoint) {
  ...
  const point = { x: start.x + deltaX * parameter, y: start.y + deltaY * parameter };
...
function segmentIntersection(
...
): { readonly point: SvgViewportPoint; readonly firstParameter: number } | undefined {
  ...
  const relative = { x: secondStart.x - firstStart.x, y: secondStart.y - firstStart.y };
  const firstParameter = (relative.x * secondDelta.y - relative.y * secondDelta.x) / denominator;
  const secondParameter = (relative.x * firstDelta.y - relative.y * firstDelta.x) / denominator;
  const epsilon = 1e-9;
  if (firstParameter <= epsilon || firstParameter >= 1 - epsilon ||
      secondParameter <= epsilon || secondParameter >= 1 - epsilon) return undefined;
  return {
    point: {
...
function interpolate(start: WorldPointXZ, end: WorldPointXZ, parameter: number): WorldPointXZ {
  return {
...
function svgDistance(first: SvgViewportPoint, second: SvgViewportPoint): number {
