...
export function useEditorSelectionActions({
  ...
  const handleAddVertexToSelectedWall = useCallback(() => {
    if (!editor.activeLevelId || !selectedEditWall || saveInteractionBlocked)
      return;
    setEditingError(undefined);
    dispatch(
      editorWallVertexPlacementStarted({
  ...
  const handleUpdateOpeningAuthoring = useCallback(
    (changes: Partial<OpeningAuthoringProperties>) => {
      ...
      const candidate =
        interaction.candidate && editor.draft && editor.activeLevelId
    ...
    if (result.ok) {
      ...
      setEditingError(
        result.errors[0]?.code ===
          ValidationErrorCode.ROOM_DISSOLUTION_AMBIGUOUS
          ? "errors.room.dissolutionAmbiguous"
          : result.errors[0]?.code ===
              ValidationErrorCode.INVALID_ROOM_DISSOLUTION
  ...
  const handleCreateLevel = useCallback(
    (properties: {
      ...
      if (!editor.draft || workspaceMode !== "edit" || saveInteractionBlocked)
        return false;
  ...
  const handleUpdateActiveLevel = useCallback(
    (properties: {
      readonly name: string;
      readonly elevation: number;
    }): boolean => {
