...
describe("ProjectApiMapper", () => {
  it("round-trips ordered Furniture with unknown definitions and detached positions", () => {
    ...
    project.building.furniture = [
      {
        ...
        definitionId: "custom-provider:item-123",
        position: { x: 50, z: 50 },
        rotation: -390.5,
        width: 120,
        depth: 60,
    ...
    expect(
      ProjectSchema.parse(JSON.parse(JSON.stringify(response.project)))
    ).toEqual(project);
    expect(response.project.building.furniture[0]?.position).not.toBe(
      ...
      [
        {
          ...
          levelCount: 2,
          roomCount: 8,
          preview: { levelId: "ground", elevation: 0, walls: [] }
        },
        {
  ...
  it("maps the complete canonical Project into a fresh response DTO graph", () => {
    ...
    expect(response.project.building.levels).not.toBe(
      canonicalProject.building.levels
    );
  ...
  it("preserves explicit Door orientation in the authoritative response", () => {
    ...
    const response = new ProjectApiMapper().toProjectResponse(
      ProjectSchema.parse(project)
    );

    expect(
      response.project.building.levels
        .flatMap((level) => level.walls)
        .flatMap((wall) => wall.openings)
  ...
  it("preserves free Room boundary geometry without inventing Wall references", () => {
    ...
    expect(room.boundary).toEqual(
      project.building.levels[0]!.rooms.at(-1)!.boundary
    );
  ...
  it("does not expose persistence ownership or technical database metadata", () => {
    const responseJson = JSON.stringify(
      new ProjectApiMapper().toProjectResponse(canonicalProject)
    );
