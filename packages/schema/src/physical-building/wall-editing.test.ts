...
describe("createWall", () => {
  it("appends a caller-identified Wall immutably and preserves unaffected state", () => {
    ...
    const result = createWall(project, { levelId: "ground-floor", wall: editableWall });
    ...
    expect(result.project.building.levels[0]?.walls.at(-1)).toEqual(editableWall);
    expect(result.project.building.levels[0]?.walls.at(-1)?.id).toBe("draft-wall");
  ...
  it("returns a typed failure for a missing Level", () => {
    const result = createWall(createProject(), { levelId: "missing-level", wall: editableWall });
  ...
  it("rejects an Opening ID already used by an Opening on the same Level", () => {
    const project = createProject();
    project.building.levels[0]?.walls[0]?.openings.push(createOpening("existing-opening"));
  ...
  it("allows cross-kind identifier reuse for new Opening IDs", () => {
    ...
    expect(result.project.building.levels[0]?.walls.at(-1)?.openings[0]?.id).toBe("existing-wall");
  });
  ...
  it("appends a valid Wall with unique Openings", () => {
    ...
    expect(
      result.project.building.levels[0]?.walls.at(-1)?.openings.map((opening) => opening.id)
...
describe("moveWallEndpoint", () => {
  it.each([
    ["start", { x: -50, z: 25 }],
    ["end", { x: 150, z: 25 }]
  ] as const)("moves the %s endpoint while preserving identity and unrelated geometry", (endpoint, position) => {
    const project = createProject();
    const before = clone(project);
    ...
    expect(result.project.building.levels[0]?.walls[1]).toBe(project.building.levels[0]?.walls[1]);
    expect(result.project.building.levels[1]).toBe(project.building.levels[1]);
  });
...
describe("deleteWall", () => {
  it("removes an unreferenced Wall immutably and preserves unrelated state", () => {
    const project = createProject();
    const before = clone(project);
    const result = deleteWall(project, { levelId: "ground-floor", wallId: "unreferenced-wall" });

    expect(result.ok).toBe(true);
    if (!result.ok) return;

    expect(project).toEqual(before);
    expect(result.project.building.levels[0]?.walls.map((wall) => wall.id)).toEqual(["existing-wall"]);
    expect(result.project.building.levels[0]?.rooms).toBe(project.building.levels[0]?.rooms);
    expect(result.project.building.levels[1]).toBe(project.building.levels[1]);
  });

  it("returns a typed failure for an unknown Wall", () => {
    expect(deleteWall(createProject(), { levelId: "ground-floor", wallId: "missing-wall" })).toMatchObject({
      ok: false,
      errors: [{ code: ValidationErrorCode.WALL_NOT_FOUND }]
    });
  });
  ...
  it("rejects deletion when a Room boundary references the Wall", () => {
    ...
    expect(deleteWall(project, { levelId: "ground-floor", wallId: "existing-wall" })).toMatchObject({
      ok: false,
